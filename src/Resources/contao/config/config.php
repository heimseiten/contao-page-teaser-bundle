...
function getImage($page_id, $page_pid, $pages, $size_id, $without_text, $teaser_only_sub_pages) {        
        ...
        if ($pages) {
            if ($teaser_only_sub_pages) {
                $arrResults = $db->executeQuery("SELECT `id`, `pid`, `title`, `pageTitle`, `description`, `teaser_headline`, `teaser_text`,".$ifPageImage." `target`, `cssClass` FROM `tl_page` WHERE pid in (". implode(',',$pages) .") AND `type`!='folder' AND `type`!='error_404' AND `hide`!='1' AND `published`='1' AND `id`!='".$page_id."' ORDER BY FIELD(pid, ". implode(',',$pages) .");")->fetchAll();    
            } else {
                $arrResults = $db->executeQuery("SELECT `id`, `title`, `pageTitle`, `description`, `teaser_headline`, `teaser_text`,".$ifPageImage." `target`, `cssClass` FROM `tl_page` WHERE id in (". implode(',',$pages) .") AND `type`!='folder' AND `type`!='error_404' AND `published`='1' AND `id`!='".$page_id."' ORDER BY FIELD(id, ". implode(',',$pages) .");")->fetchAll();    
            }
        } else {
            ...
            echo '<div class="ce_teaser_wrapper ' . $arrResults[$i]['cssClass'] . '" '.$style.'>';
                echo '<div class="inside">
                    ...
                    if ($arrResults[$i]['pageImage']) {
                        $fileModel = \Contao\FilesModel::findByUuid($arrResults[$i]['pageImage']);
                        if ($fileModel) {
                            echo '{{picture::'.$fileModel->path.'?size='.$size_id.'}}';
                        }
                    } else {
                        ...
                        if ($tl_article[0]['id']) {
                            ...
                            if ($tl_content[0]['singleSRC']!='') {
                                $fileModel = \Contao\FilesModel::findByUuid($tl_content[0]['singleSRC']);
                                if ($fileModel) {
                                    echo('{{picture::'.$fileModel->path.'?size='.$size_id.'}}');
                                }
                            } else {
