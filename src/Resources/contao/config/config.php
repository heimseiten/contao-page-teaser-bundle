...
function getImage($page_id, $page_pid, $pages, $size_id, $without_text, $teaser_only_sub_pages) {    
    if (TL_MODE == "FE") {
        if ($pages) {
            ...
            $arrResults = \Database::getInstance()->query(    "SELECT id, title, pageTitle, description,teaser_headline, teaser_text, pageImage FROM `tl_page` WHERE pid =". $page_id ." AND `type`!='folder' AND `type`!='error_404' AND `hide`!='1' AND `published`='1' ORDER BY `sorting`;")->fetchAllAssoc();    
            if ($arrResults == NULL){ 
                $arrResults = \Database::getInstance()->query("SELECT id, pid, title, pageTitle, description,teaser_headline, teaser_text, pageImage FROM `tl_page` WHERE pid =". $page_pid ." AND `type`!='folder' AND `type`!='error_404' AND `hide`!='1' AND `published`='1' AND `id`!='".$page_id."' ORDER BY `sorting`;")->fetchAllAssoc();    
            }            
        }
...
function renderPageImage($pageImage) {
